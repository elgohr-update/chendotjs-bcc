...
int trace_ip_route_output_key_hash(struct pt_regs *ctx, struct net *net, struct flowi4 *flp, struct sk_buff *skb)
{
    if (flp == NULL)
        return 0;
    ...
    if (true) {
        ...
        data4.skbaddr = (unsigned long long)(skb);
...
# initialize BPF
b = BPF(text=bpf_text)
if b.get_kprobe_functions(b"ip_route_output_key_hash"):
    b.attach_kprobe(event="ip_route_output_key_hash", fn_name="trace_ip_route_output_key_hash")
else:
    print("ERROR: ip_route_output_key_hash() kernel function not found or traceable. "
