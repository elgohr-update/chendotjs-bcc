...
int trace_fib_select_path(struct pt_regs *ctx, struct net *net, struct fib_result *res, struct flowi4 *flp, const struct sk_buff *skb)
{
    if (flp == NULL)
        return 0;
...
# initialize BPF
b = BPF(text=bpf_text)
if b.get_kprobe_functions(b"fib_select_path"):
    b.attach_kprobe(event="fib_select_path", fn_name="trace_fib_select_path")
else:
    print("ERROR: fib_select_path() kernel function not found or traceable. "
